 ...
 */
/**
 * GET /developers/passport/profile?agentId=&serviceId=
 * Provider evidence (Agent Passport) for the marketplace evidence modal and
 * the agent-profile page. Trust data is Graph-derived; see getPassportProfile.
 */
export const passportProfile = async (req, res) => {
  try {
    const { agentId, serviceId } = req.query;
    ok(res, await getPassportProfile({ agentId, serviceId }));
  } catch (err) {
    handleError(res, err, 'passport');
  }
};
