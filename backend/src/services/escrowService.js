...
export const createEscrow = async ({ sessionId, serviceId, buyerAgentId, sellerAgentId, amountWei, amountBOT, invoiceId }) => {
  ...
  return {
    escrowId: 'esc_' + crypto.randomBytes(12).toString('hex'),
...
export const releaseEscrow = async ({ sessionId }) => {
  ...
  return {
    ...
    message: `Escrow marked released in Arbiter ledger (off-chain bookkeeping; no on-chain transfer in this release).`
  };
};
...
export const refundEscrow = async ({ sessionId, reason }) => {
  ...
  return {
    ...
    message: `Escrow marked refunded in Arbiter ledger (off-chain bookkeeping; no on-chain transfer in this release).`
  };
};
