/**
 * Live The Graph data access for Arbiter's Trust Engine.
 *
...
const NETWORK = process.env.GRAPH_NETWORK || 'Arbitrum Sepolia';
...
const graphQuery = async (query, variables = {}) => {
  ...
  // A retired/unknown deployment answers 200 with no `data` (e.g. {"message":"Not found"}).
  // Treating that as an empty snapshot would silently zero the Trust Engine.
  if (!body.data) {
    throw new GraphUnavailableError('The Graph endpoint returned no data (deployment missing or retired).');
  }
  return body.data;
};
...
// ==================== On-chain event fallback ====================
// The Studio subgraph is the preferred source, but until it is (re)deployed
// against Arbitrum Sepolia, the Trust Engine reads the SAME entities straight
// from ArbiterManager event logs — no fabricated data, no Postgres trust.

const MANAGER_ADDRESS = String(process.env.ARBITER_MANAGER_ADDRESS || '').toLowerCase();
// First ArbiterManager log lands at 310961269; start a little earlier for safety.
const DEPLOY_BLOCK = Number(process.env.ARBITER_DEPLOY_BLOCK || 310955000);
const PAYMENT_TYPE_NAMES = ['DIRECT', 'SCHEDULED', 'ESCROW', 'INVOICE', 'REQUEST'];
// Outcome: NONE=0 APPROVED=1 PARTIAL=2 REJECTED=3 AUTO_DELIVERED=4 AUTO_REFUNDED=5
const OUTCOME_TERMINAL = new Set([1, 2, 4]); // resolved in the provider's favour
const OUTCOME_REFUND = new Set([3, 5]);      // money went back to the buyer

let lastSnapshotSource = null; // 'subgraph' | 'onchain-events'
export const getSnapshotSource = () => lastSnapshotSource;

const arbiterInterface = new ethers.Interface([
...
const CREATED_TOPIC = arbiterInterface.getEvent('PaymentCreated').topicHash;
const COMPLETED_TOPIC = arbiterInterface.getEvent('SettlementCompleted').topicHash;

/** Bounded log scan — chunked so public RPC range limits never trip. */
const fetchArbiterLogs = async (topics, fromBlock, toBlock) => {
  const provider = getProvider();
  const CHUNK = 50_000;
  const logs = [];
  for (let start = fromBlock; start <= toBlock; start += CHUNK) {
    const end = Math.min(start + CHUNK - 1, toBlock);
    const batch = await provider.getLogs({ address: MANAGER_ADDRESS, topics, fromBlock: start, toBlock: end });
    logs.push(...batch);
  }
  return logs;
};

const parseArbiterLog = (log, blockTimestamps) => ({
  id: log.args.id,
  transactionHash: log.transactionHash,
  blockNumber: String(log.blockNumber),
  timestamp: String(blockTimestamps.get(log.blockNumber) ?? 0),
  payer: log.args.sender || log.args.receiver,
  payee: log.args.receiver,
  // On-chain USDC is 6-decimal; the pipeline expects 18-decimal strings.
  // Scale at the boundary so amount() (÷1e18) stays untouched downstream.
  amount: (log.args.amount != null ? log.args.amount * 10n ** 12n : 0n).toString(),
  paymentType: log.args.pType != null ? (PAYMENT_TYPE_NAMES[Number(log.args.pType)] || 'ESCROW') : 'ESCROW',
  status: log.args.outcome != null
    ? (OUTCOME_REFUND.has(Number(log.args.outcome)) ? 'CANCELLED' : 'RELEASED')
    : 'HELD',
  releaseTime: log.args.releaseTime ? String(log.args.releaseTime) : null
});

/** Build the same payment snapshot the subgraph would serve, from raw logs. */
const loadOnChainSnapshotRaw = async () => {
  if (!MANAGER_ADDRESS) throw new GraphUnavailableError('ARBITER_MANAGER_ADDRESS is not configured.');
  const provider = getProvider();
  const headBlock = await provider.getBlockNumber();
  const logs = await fetchArbiterLogs([CREATED_TOPIC], DEPLOY_BLOCK, headBlock);
  logs.push(...await fetchArbiterLogs([COMPLETED_TOPIC], DEPLOY_BLOCK, headBlock));
  ...
  const parsed = logs
    .map((log) => {
      try { return arbiterInterface.parseLog(log); } catch { return null; }
    })
    .filter(Boolean)
    .map((parsedLog) => parseArbiterLog(parsedLog, blockTimestamps));

  // Fold created + completed into one row per payment id (completion wins).
  const byId = new Map();
  for (const row of parsed) {
    const existing = byId.get(row.id);
    byId.set(row.id, existing && existing.status !== 'HELD' ? existing : row);
  }
  const payments = Array.from(byId.values()).sort((a, b) => Number(a.timestamp) - Number(b.timestamp));
  lastSnapshotSource = 'onchain-events';
  logger.info(`[GRAPH] Subgraph unavailable — Trust Engine served ${payments.length} payment(s) from ArbiterManager logs (on-chain fallback).`);
  return {
    meta: { block: { number: String(headBlock) }, deployment: 'onchain-events' },
    payments
  };
};

/** Subgraph first; if it is unreachable/unconfigured, fall back to chain logs. */
const loadGraphSnapshotRaw = async () => {
  try {
    const snapshot = await loadSubgraphSnapshotRaw();
    lastSnapshotSource = 'subgraph';
    return snapshot;
  } catch (err) {
    if (!(err instanceof GraphUnavailableError)) throw err;
    return loadOnChainSnapshotRaw();
  }
};

/** Paginated snapshot — never silently truncated at 1,000 rows. */
const loadSubgraphSnapshotRaw = async () => {
  const payments = [];
  let lastId = null;
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    if (isFirst && !data._meta) {
      throw new GraphUnavailableError('The Graph deployment is not indexed at this URL.');
    }
    payments.push(...batch);
...
const loadSettlementsAndInvoices = async () => {
  try {
    const data = await graphQuery(`
      query ArbiterSideTables($first: Int!) {
    ...
    if (!(err instanceof GraphUnavailableError)) throw err;
    // On-chain fallback mode: side tables are derived from the same logs.
    const snapshot = await loadGraphSnapshot();
    const settled = snapshot.payments.filter((p) => p.status !== 'HELD');
    return { settlements: settled, invoiceReferences: [] };
  }
};
 ...
 *
 * `publisherContext` carries Arbiter's publisher-level identity model:
 *   { humanVerified, walletCount, aggregatedSettlementVolume, firstSettlement,
 *     inheritsReputation, siblingWallets }
 * Publisher continuity acts as a bounded trust floor (never a multiplier),
...
const providerIntelligence = (payee, payments, humanBacked = false, publisherContext = null) => {
  ...
  // Publisher continuity acts as a trust floor: an established publisher's
  ...
  // Publisher continuity (Arbiter identity model): a newly registered wallet
  ...
  if (inheritsPublisherReputation) {
    reasoning.push(`Publisher continuity: new wallet inherits the publisher's record — ${publisherContext.walletCount} wallet(s), ${(publisherContext.aggregatedSettlementVolume || 0).toFixed(4)} USDC combined volume`);
  }
...
export const getGraphStatus = async () => {
  ...
  return {
    ...
    recentSettlements: settlements.slice(0, 6).map((s) => ({
      transactionHash: s.transactionHash,
      timestamp: s.timestamp,
      amount: amount(s.amount),
      status: s.status
    })),
    // Full settlement series (most recent first) — feeds the dashboard's
    // Revenue / USDC Volume charts with real on-chain history even when the
    // local invoice tables have no rows yet (fresh workspaces).
    settlementSeries: settlements.map((s) => ({
      transactionHash: s.transactionHash,
      timestamp: s.timestamp,
      amount: amount(s.amount),
      status: s.status
...
/**
 * Arbiter publisher-identity context — the continuity layer.
 * All wallets owned by one publisher/organization are grouped into a single
 * publisher profile; reputation aggregates at the publisher level so a new
 * agent wallet inherits the publisher's settlement track record.
 */
export const buildPublisherContext = async ({ developerId, organizationId, currentWallet }) => {
  ...
  const snapshot = await loadGraphSnapshot();
  const walletSet = new Set(wallets.map((a) => String(a.wallet_address).toLowerCase()));
...
export const analyzeProvider = async (providerId) => {
  ...
  try {
    const { data: agentRow } = await supabase
      .from('ai_agents')
      .select('developer_id, organization_id, agent_name')
  ...
  return { ...providerIntelligence(addr, snapshot.payments, false, publisherContext), agentName };
};
...
export const analyzeProviders = async ({ providerIds = [] } = {}) => {
  ...
  try {
    const { data: agents } = await supabase
      .from('ai_agents')
      .select('wallet_address, developer_id, organization_id, agent_name')
  ...
  if (!agentRows.length && payees.length) {
    try {
      const { rows } = await getPool().query(
        'SELECT wallet_address, developer_id, organization_id, agent_name FROM ai_agents WHERE LOWER(wallet_address) = ANY($1)',
  ...
  return payees
    .map((payee) => ({ ...providerIntelligence(payee, snapshot.payments, false, publisherContextMap.get(payee) || null), agentName: nameMap.get(payee) || null }))
...
export const verifySettlement = async (txHash) => {
  ...
  try {
    ...
    if (!(err instanceof GraphUnavailableError)) throw err;
    const snapshot = await loadGraphSnapshot();
    const row = snapshot.payments.find((p) => String(p.transactionHash).toLowerCase() === String(txHash).toLowerCase()) || null;
    return { verified: Boolean(row), settlement: row, source: 'ArbiterManager event logs' };
  }
};
...
export const getPaymentEntity = async (paymentId) => {
  try {
    ...
    if (!(err instanceof GraphUnavailableError)) throw err;
    const snapshot = await loadGraphSnapshot();
    return snapshot.payments.find((p) => String(p.id).toLowerCase() === String(paymentId).toLowerCase()) || null;
  }
};
