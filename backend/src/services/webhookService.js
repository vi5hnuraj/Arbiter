...
const SIGNATURE_HEADER = 'x-arbiter-signature';
const TIMESTAMP_HEADER = 'x-arbiter-timestamp';
...
/**
 * Verify an incoming x-arbiter-signature header against a known secret.
...
const deliver = async (endpoint, event, payload) => {
  ...
  const headers = {
    ...
    'x-arbiter-request-id': requestId,
    'x-arbiter-event': event,
    'x-arbiter-webhook-id': endpoint.id,
...
export const getDelivery = async ({ organizationId, deliveryId }) => {
  ...
  return {
    ...
    signatureHeader: 'x-arbiter-signature',
    timestampHeader: 'x-arbiter-timestamp',
    durationMs: d.duration_ms,
    ...
    sentHeaders: {
      'Content-Type': 'application/json',
      'x-arbiter-event': d.event,
      'x-arbiter-request-id': d.request_id || null,
      'x-arbiter-timestamp': d.last_attempt_at ? Math.floor(new Date(d.last_attempt_at).getTime() / 1000).toString() : null,
      'x-arbiter-signature': d.signature ? `${d.signature}` : null
    }
  };
};
