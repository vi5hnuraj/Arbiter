  ...
  switch (name) {
    ...
    case "checkBalance": {
      ...
      try {
        ...
        const rpcUrl = process.env.ARC_RPC_URL || process.env.RPC_URL || "https://sepolia-rollup.arbitrum.io/rpc";
    ...
    case "getTransactionHistory": {
      ...
      const explorerUrl = process.env.ARC_EXPLORER_URL || process.env.EXPLORER_URL || "https://sepolia.arbiscan.io";
    ...
    case "findUser": {
      ...
      if (!foundUser) {
        return {
          tool: "findUser",
          success: false,
          message: `🔍 User '${query}' not found on Arbiter.`
        };
      }
    ...
    case "cancelSchedulePayment": {
      ...
      const cancelOne = (t, state) => {
        ...
        return {
          ...
          message: `🔍 Found scheduled transfer #${t.id} (${t.status}) to ${t.receiver_pay_tag || 'Unknown'} for ${Number(t.bot_amount || t.amount || 0)} USDC.\n\nTo cancel and refund, click "Cancel & Refund" below. This will send a cancel transaction to the Arbiter Manager contract via MetaMask.`
        };
      };
