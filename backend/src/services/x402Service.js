/**
 * x402 Service — HTTP 402 (Payment Required) protocol support for Arbiter.
 *
 ...
 *                events — this table is Arbiter's own accounting mirror.
 */
...
export const buildChallenge = async ({ endpoint, price }) => {
  ...
  return {
    ...
    payment: {
      ...
      recipient: recipient || 'Arbiter Treasury',
      recipientAddress: recipient || null,
      purpose: 'Premium API',
      endpoint,
      instructions: {
        asset: 'USDC (native gas on Arc)',
        rail: 'Arbiter embedded wallet (Privy) → recipient (native transfer)',
        header: 'X-PAYMENT',
