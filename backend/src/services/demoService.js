 ...
 * 3. Trust Engine check (top provider readiness)
...
export const runAutonomousDemo = async ({ developerId, organizationId } = {}) => {
  ...
  try {
    ...
    const { data: agents } = await supabase
      .from('ai_agents')
      .select('id, agent_id, agent_name, wallet_id, developer_id, organization_id, wallet_address')
    ...
    try {
      ...
      if (settlement.success) {
        addStage('settlement', 'complete', `Payment settled: ${settlement.txHash}`, {
          ...
          explorerUrl: `https://sepolia.arbiscan.io/tx/${settlement.txHash}`
        });
      } else {
