...
import { isTokenMode } from './settlementToken.js';
...
export const enrichServices = async (services) => {
  ...
  const [{ data: caps }, { data: reps }, { data: agents }] = await Promise.all([
    ...
    supabase.from('ai_agents').select('id, human_backed').in('id', agentIds)
  ]);
  ...
  return services.map((s) => ({
    ...
    onChainRegistered: agentMap.get(s.agent_id)?.human_backed || false
  }));
};
...
export const inferRequirements = (task = '', requirement) => {
  ...
  if (req.verifiedOnly == null && /\bverif(?:ied|y)|human[- ]backed|on-chain rep/.test(t)) req.verifiedOnly = true;
...
export const recommendProviders = async ({ developerId, organizationId, consumerAgent, task, requirements }) => {
  ...
  for (const s of enriched) {
    ...
    if (req.verifiedOnly && !s.onChainRegistered) { block('verified'); continue; }
  ...
  return {
    ...
    recommended: ranked.map(({ c, confidence }) => ({
      ...
      estimatedCompletionMs: estMs(c),
       ...
       ,trustSource: c.graphTrust?.source || 'Arbiter database'
...
const toPublicSession = (s) => ({
  ...
  explorerUrl: s.payment_tx_hash ? `${isKeeperHubRail() ? (process.env.KEEPERHUB_EXPLORER_URL || 'https://sepolia.arbiscan.io') : EXPLORER_URL}/tx/${s.payment_tx_hash}` : null,
...
export const confirmPrepaidPurchase = async ({ sessionId, organizationId }) => {
  ...
  let balance;
  if (!useKeeperHubRail && isTokenMode()) {
    // ── Stablecoin gate: the buyer's USDC/USDG balance must cover the purchase
    // (native ETH is only needed for gas, checked by the RPC at send time). ──
    const { getSettlementToken, weiToTokenUnits } = await import('./settlementToken.js');
    const { kind: gateTokenKind, address: gateTokenAddress } = getSettlementToken();
    let tokenUnits = null;
    try {
      const rpc = new ethers.JsonRpcProvider(process.env.ARC_RPC_URL || 'https://sepolia-rollup.arbitrum.io/rpc');
      const tk20 = new ethers.Contract(gateTokenAddress, ['function balanceOf(address) view returns (uint256)'], rpc);
      tokenUnits = await tk20.balanceOf(consumer.wallet_address);
    } catch (err) {
      // Fail-open on the READ: if the balance can't be fetched, let the chain
      // itself reject the settle (it enforces the same rule).
      logger.warn('[COMMERCE] token balance gate skipped (read failed):', err.message);
    }
    const needUnits = BigInt(weiToTokenUnits(amountWei));
    if (tokenUnits !== null && tokenUnits < needUnits) {
      const failedSession = await setSessionPaymentFailed(session, `Insufficient ${gateTokenKind} balance`, provider);
      return {
        success: false,
        failed: true,
        failureReason: `Insufficient ${gateTokenKind} balance`,
        requiredBOT: amountBOT,
        availableBOT: Number(ethers.formatUnits(tokenUnits, 6)),
        session: toPublicSession(failedSession),
        invoice: null,
        credits: 0
      };
    }
    balance = { wei: '0', formatted: tokenUnits ? String(Number(ethers.formatUnits(tokenUnits, 6))) : '0' };
  } else {
    try {
  ...
  // Arbiter keeps marketplace% as commission, provider gets the rest.
  ...
  let releaseTxHash;
  try {
    const managerAddress = process.env.ARBITER_MANAGER_ADDRESS || '0x6b5BA9E3E8175eCfB8291FA0ee358b070c0522Cc';
    paymentId = ethers.keccak256(ethers.toUtf8Bytes(`arbiter:purchase:${session.session_id}`));
    invoiceReference = ethers.keccak256(ethers.toUtf8Bytes(`arbiter:invoice:${session.session_id}`));
    ...
    if (useKeeperHubRail) {
      ...
      const keeperhubResult = await keeperHubSettlePurchase({
        ...
        // ARBITER_MANAGER_ADDRESS is untouched.
      ...
      // ── Direct rail, stablecoin mode (default): real USDC/USDG escrow via
      // the ERC20 path of the SAME ArbiterManager — identical to the x402 held
      // flow, so every purchase settles in the prize-native asset. ──
      const { getSettlementToken, weiToTokenUnits } = await import('./settlementToken.js');
      const { kind: tokenKind, address: tokenAddress } = getSettlementToken();
      const amountUnits = weiToTokenUnits(amountWei); // 18dp wei → 6dp token units
      ...
      const approve = await walletService.sendContractCall({
        walletId: consumer.wallet_id,
        to: tokenAddress,
        data: erc20.encodeFunctionData('approve', [managerAddress, amountUnits]),
        idempotencyKey: `invoice-approve:${session.session_id}`
      });
      const create = await walletService.sendContractCall({
        walletId: consumer.wallet_id,
        to: managerAddress,
        data: managerInterface.encodeFunctionData('settleInvoiceToken', [paymentId, provider.wallet_address, invoiceReference, tokenAddress, amountUnits]),
      ...
      const release = await walletService.sendContractCall({
        walletId: consumer.wallet_id,
        to: managerAddress,
        data: managerInterface.encodeFunctionData('releaseToken', [paymentId]),
      ...
      // ── Direct rail, native-ETH mode (COMMERCE_ASSET=native) ──
  ...
  // Feed the completed Arc action back into Arbiter's local reputation
  ...
  const { data: invRow, error: invErr } = await supabase
    ...
    .insert({
      ...
      quantity: String(session.quantity),
      unit: session.unit || null,
      amount_wei: amountWei,        currency: result.asset && result.asset !== 'NATIVE' && result.asset !== 'ETH' ? result.asset : 'ETH',
      status: 'paid',
        tx_hash: txHash,
      paid_at: paidAt,
        metadata: {
  ...
  return {
    ...
    execution: result.rail === 'keeperhub' ? {
      ...
      explorerUrls: [createTxHash, releaseTxHash].filter(Boolean).map((h) => `https://sepolia.arbiscan.io/tx/${h}`),
