/**
 * AI Assistant Service — the natural-language interface to Arbiter.
 *
 * Interprets a human request, selects the appropriate automation recipe,
...
const handlePurchase = async (text, { consumerAgentId, developerId, organizationId }, tracker) => {
  ...
  const answer = [
    ...
    `Explorer: https://sepolia.arbiscan.io/tx/${settlement.txHash}`,
    '',
  ...
  return {
    ...
    data: {
      ...
      settlement: { txHash: settlement.txHash, amount: settlement.amountBOT, explorerUrl: `https://sepolia.arbiscan.io/tx/${settlement.txHash}` },
...
const handleFindProviders = async (text, tracker) => {
  ...
  const providerLines = topProviders.map((p, i) => {
    return `${i + 1}. Trust ${p.trustScore}/100 · ${p.successfulPayments}/${p.paymentCount} successful · ${p.settlementVolume?.toFixed(4)} USDC · ${p.uniquePayers} buyer(s) · risk ${p.riskLevel}`;
  }).join('\n\n');

  return {
    message: topProviders.length ? result.answer : 'No matching providers have indexed Graph settlement evidence yet.',
    answer: topProviders.length ? result.answer : 'No matching providers have indexed settlement evidence on The Graph yet. Arbiter will not recommend a provider without verifiable on-chain history.',
    steps: tracker.steps,
    data: {
...
const handleCheckVerified = async (tracker, consumerAgentId) => {
  tracker.add('observe', 'Checking agent status', 'Querying agent registry...');
  let query = supabase.from('ai_agents').select('agent_id, agent_name, wallet_address');
  ...
  return { message: 'Agent ready.', answer: `✅ Your agent ${agent.agent_name || agent.agent_id} is active.\n\n• Wallet: ${agent.wallet_address?.slice(0, 10)}…\n\nPublishing is open — services are gated by API scopes, and The Graph measures settlement trust.`, steps: tracker.steps };
};

const handleWhyCantPublish = async (tracker) => {
  tracker.add('observe', 'Checking publish readiness', 'Querying agent setup...');
  const { data: agents } = await supabase.from('ai_agents').select('agent_id, agent_name').limit(1);
  const agent = agents?.[0];
  if (!agent) return { message: 'No agent found.', answer: '❌ Create an AI agent first — then you can publish a service.', steps: tracker.steps };
  return { message: 'Publishing is open.', answer: `✅ Nothing is blocking publishing.\n\n1. Go to /developer/marketplace/services/publish\n2. Describe the service and set the price\n3. Publish — The Graph scores your settlements as you serve buyers`, steps: tracker.steps };
};

const handleWhatDoINeed = async (tracker) => {
  const { data: agents } = await supabase.from('ai_agents').select('agent_id, agent_name, wallet_address').limit(1);
  const agent = agents?.[0];
  const checks = [];
...
const handleShowServices = async (tracker) => {
  ...
  if (!services?.length) return { message: 'No published services.', answer: '📭 No services published yet. Publish your first AI service from the Services page.', steps: tracker.steps };
...
const handleHelp = (tracker) => ({
  message: 'I can help you interact with Arbiter using natural language.',
  answer: `🤖 Arbiter AI Assistant
...
Every response explains WHY — backed by The Graph and Arbiter on-chain escrow.`,
  steps: tracker.steps,
  data: null
});
