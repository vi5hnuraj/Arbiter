...
import { analyzeProviders } from './graphIntelligenceService.js';

// ==================== Live balance hydration ====================

const LIVE_BALANCE_CONCURRENCY = 16;
const LIVE_BALANCE_TIMEOUT_MS = 5000;

/**
 * Replace the cached ai_agents.balance column with the live on-chain USDC
 * balance for a page of agent rows.
 *
 * The cache is only written by the single-agent balance endpoint, so a faucet
 * deposit, an escrow release, or an x402 payment leaves every list view (the
 * publish wizard, Agent Studio, dashboards) showing the previous number — a
 * funded agent reads 0 and a drained one still reads its old balance.
 *
 * Reads run in parallel and are individually bounded: on RPC failure or
 * timeout the row keeps its cached value, so a dead RPC degrades to stale data
 * rather than failing the request. Refreshed values are written back so other
 * readers pick them up too (best-effort — write failures never affect the read).
 */
const withLiveBalances = async (rows) => {
  const targets = rows.filter((r) => r.wallet);
  ...
  const readOne = async (row) => {
    try {
      const timeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('balance read timeout')), LIVE_BALANCE_TIMEOUT_MS)
      );
      const bal = await Promise.race([walletService.getBalance(row.wallet), timeout]);
      const value = Number(bal?.formatted);
      if (!Number.isFinite(value)) return null;
      return { row, balance: value.toFixed(6) };
    } catch {
      return null; // keep cached value
    }
  };

  const resolved = [];
  for (let i = 0; i < targets.length; i += LIVE_BALANCE_CONCURRENCY) {
    resolved.push(...(await Promise.all(targets.slice(i, i + LIVE_BALANCE_CONCURRENCY).map(readOne))));
  }

  // Write back only the rows whose value actually moved.
  await Promise.allSettled(
    resolved
      .filter((r) => r && String(r.row.balance ?? '') !== r.balance)
      .map((r) =>
        supabase
          .from('ai_agents')
          .update({ balance: r.balance, updated_at: new Date().toISOString() })
          .eq('id', r.row.rawId)
      )
  );

  const byWallet = new Map(resolved.filter(Boolean).map((r) => [r.row.wallet.toLowerCase(), r.balance]));
  return rows.map((r) => {
    const live = r.wallet ? byWallet.get(r.wallet.toLowerCase()) : undefined;
    return live !== undefined ? { ...r, balance: live } : r;
  });
};
 ...
 * 2. Send 4.9 USDC to treasury (100% to Arbiter)
...
export const paySubscriptionWithBOT = async ({ organizationId, developerId, walletId, txHash: externalTxHash }) => {
  ...
  if (walletId && walletId.startsWith('metamask-')) {
    ...
    if (tx.to.toLowerCase() !== treasuryAddress.toLowerCase()) {
      throw new Error('Transaction recipient is not Arbiter treasury.');
    }
  ...
  // === AGENT WALLET PAYMENT ===
    ...
    // 2. Send payment to treasury (100% to Arbiter)
    try {
...
export const listAgentsWithStats = async (orgId, developerId, { search, status, page = 1, perPage = 10 } = {}) => {
  ...
  let rows = agents.map((a) => {
    ...
    return {
      // DB row id — lets withLiveBalances() persist a refreshed balance.
      // Stripped from the response below so it never reaches the client.
      rawId: a.id,
      agentId: a.agent_id,
      name: a.agent_name,
  ...
  // Hydrate only the page we are about to return — bounded RPC work per request.
  const pageRows = await withLiveBalances(rows.slice(start, start + perPage));
  ...
  return {
    ...
    perPage,
    totalPages,
    agents: pageRows.map(({ rawId, ...rest }) => rest)
  };
};
...
// ==================== Agent Passport (provider evidence) ====================

const PASSPORT_SERVICE_COLS =
  'id, service_id, agent_id, agent_code, title, category, pricing_model, unit_price, unit_label, is_active, require_x402, x402_price, created_at';

// Used only when the agent has no wallet yet, so the page can render an honest
// "new provider" passport instead of failing. Never used to fake a score.
const PASSPORT_ZERO_INTELLIGENCE = {
  trustScore: 0,
  confidence: 0,
  riskLevel: 'unknown',
  riskFlags: [],
...
const findPassportAgent = async (agentCode) => {
  if (!agentCode) return null;
  try {
    const { data, error } = await supabase
      .from('ai_agents')
      .select('id, agent_id, agent_name, wallet_address, organization_id, human_backed')
      .eq('agent_id', agentCode)
      .maybeSingle();
    if (!error && data) return data;
  } catch { /* gateway degraded — fall through */ }
  try {
    const { rows } = await getPool().query(
      'SELECT id, agent_id, agent_name, wallet_address, organization_id, human_backed FROM ai_agents WHERE agent_id = $1 LIMIT 1',
      [agentCode]
    );
...
const findPassportService = async (serviceCode) => {
  if (!serviceCode) return null;
  try {
    const { data, error } = await supabase
      .from('ai_services')
      .select(PASSPORT_SERVICE_COLS)
  ...
  try {
    const { rows } = await getPool().query(
      `SELECT ${PASSPORT_SERVICE_COLS} FROM ai_services WHERE service_id = $1 LIMIT 1`,
      [serviceCode]
    );
    if (rows.length) return rows[0];
  } catch { /* direct read unavailable */ }
...
/**
 * Agent Passport — the provider-evidence payload behind the marketplace's
 * "View provider evidence" action and the /developer/agent-profile page.
 *
 * The score comes exclusively from The Graph (analyzeProviders); Postgres is
 * read only for identity (agent_id → wallet/name/publisher) and the provider's
 * published services. Lookups are intentionally NOT org-scoped — a buyer
 * inspects a provider it does not own.
 *
 * Graph failures propagate as GRAPH_UNAVAILABLE (503) rather than rendering a
 * fabricated 0/100: an honest error beats a misleading score.
 */
export const getPassportProfile = async ({ agentId, serviceId } = {}) => {
  const service = serviceId ? await findPassportService(serviceId) : null;
  let agent = await findPassportAgent(agentId);
  if (!agent && service?.agent_code) agent = await findPassportAgent(service.agent_code);
  if (!agent) throw Object.assign(new Error('Provider not found.'), { status: 404 });

  const analysed = await analyzeProviders({ providerIds: [agent.agent_id] });
  const intel = analysed[0] || PASSPORT_ZERO_INTELLIGENCE;
  ...
  try {
    const { data, error } = await supabase
      ...
      .order('created_at', { ascending: false });
    if (!error && data) capabilities = data;
  } catch { /* gateway degraded — fall through */ }
  if (!capabilities.length) {
    try {
      const { rows } = await getPool().query(
        `SELECT ${PASSPORT_SERVICE_COLS} FROM ai_services WHERE agent_id = $1 ORDER BY created_at DESC`,
        [agent.id]
      );
      capabilities = rows;
    } catch { /* direct read unavailable */ }
  ...
  // Publisher display name: the owning organization when we can resolve it.
  let publisherName = agent.agent_name || 'Verified publisher';
  if (agent.organization_id) {
    try {
      const { data: org } = await supabase
        ...
        .maybeSingle();
      if (org?.name) publisherName = org.name;
    } catch { /* keep the agent name */ }
  }

  const pub = intel.publisher || null;
  const paymentCount = Number(intel.paymentCount ?? 0);

  const passport = {
    agentId: agent.agent_id,
    agentName: agent.agent_name || null,
    passportId: agent.wallet_address
      ? `PP-${String(agent.wallet_address).slice(2, 10).toUpperCase()}`
      : agent.agent_id,
    walletAddress: agent.wallet_address || null,
    trustScore: Number(intel.trustScore ?? 0),
    trustSource: intel.source || 'The Graph',
    riskLevel: intel.riskLevel || 'unknown',
    riskFlags: intel.riskFlags || [],
    trustReasoning: intel.reasoning || [],
    successRate: Number(intel.successRate ?? 0),
    confidence: Number(intel.confidence ?? 0),
    onChainRegistered: paymentCount > 0,
    humanBacked: Boolean(agent.human_backed),
    intelligence: {
      paymentCount,
      successfulPayments: Number(intel.successfulPayments ?? 0),
      settlementVolume: Number(intel.settlementVolume ?? 0),
      uniqueBuyers: Number(intel.uniquePayers ?? 0),
      repeatBuyers: Number(intel.repeatCustomers ?? 0),
      lastPaymentAt: intel.lastSettlement || null,
      lastSettlement: intel.lastSettlement || null
    },
    publisher: { name: publisherName, walletCount: pub?.walletCount || 1 },
    continuity: {
      active: Boolean(pub?.inheritsReputation),
      explanation: pub?.inheritsReputation
        ? `This wallet belongs to ${publisherName}, whose ${pub.walletCount} wallets share one publisher record — a new wallet inherits that settlement history instead of starting from zero.`
        : 'Reputation continuity links wallets owned by the same publisher, so trust earned on one wallet follows the publisher.',
      publisherWalletCount: pub?.walletCount || 1,
      aggregatedSettlementVolume: Number(pub?.aggregatedSettlementVolume ?? 0),
      firstSettlement: pub?.firstSettlement
        ? new Date(Number(pub.firstSettlement) * 1000).toISOString()
        : null
    },
    // settlements are already amount()-normalised to USDC units.
    paymentHistory: (intel.settlements || []).slice(0, 10),
    capabilities: capabilities.map((c) => ({
      ...
      requireX402: Boolean(c.require_x402),
      x402Price: c.x402_price || null
    }))
  };

  // Only echo the service when it really belongs to the resolved provider.
  const serviceMatches = service && service.agent_code === agent.agent_id;

  return {
    passport,
    service: serviceMatches
      ? { serviceId: service.service_id, title: service.title, category: service.category }
      : null
  };
};
