    ...
    try {
      ...
      for (const payment of duePayments) {
        try {
          ...
          let onChain;
          try {
            onChain = await contract.getPayment(paymentId);
          } catch (decodeErr) {
            // BAD_DATA = paymentId doesn't exist on THIS chain's contract (e.g.
            // legacy Base-era rows after the Arbitrum migration). They can never
            // resolve here — mark CANCELLED so the poll loop skips them forever.
            if (decodeErr?.code === 'BAD_DATA' || String(decodeErr?.message || '').includes('could not decode')) {
              await sqlUpdateTransfer(payment.id, { status: 'CANCELLED' });
              logger.warn(`⏰ [SCHEDULER WORKER] Payment ${payment.id}: paymentId not on this chain (legacy row) — marked CANCELLED`);
              continue;
            }
            throw decodeErr;
          }
