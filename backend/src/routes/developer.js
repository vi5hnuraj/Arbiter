...
// Provider evidence (Agent Passport) — Graph-derived trust for a provider the
// caller does NOT own, so it is scoped by marketplace read, not agent read.
router.get('/passport/profile', ...guard(SCOPES.SERVICES_READ, 'marketplace.read'), usageMiddleware, passportProfile);
...
router.post('/webhooks/:id/rotate', ...guard([SCOPES.WEBHOOKS_MANAGE, SCOPES.WEBHOOKS_UPDATE], 'webhooks.manage'), usageMiddleware, rotate);
  ...
  router.post('/webhooks/test', ...guard([SCOPES.WEBHOOKS_MANAGE, SCOPES.WEBHOOKS_CREATE], 'webhooks.manage'), usageMiddleware, async (req, res) => {
    try {
      ...
      const payload = req.body?.payload || { test: true, timestamp: new Date().toISOString(), message: 'Arbiter webhook test event' };
