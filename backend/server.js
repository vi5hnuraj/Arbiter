...
import heldRoutes from './src/routes/heldRoutes.js'; // ✅ Held-mode x402: pay into escrow, read first, decide after
import arbiterRoutes from './src/routes/arbiterRoutes.js'; // ✅ Console demos: B2B engagement cycle + held panel + reputation
...
dotenv.config({ override: true }); // .env wins over shell env (PORT=0 is exported globally in some environments)
...
app.use('/api/x402/held', heldRoutes); // ✅ escrow-by-substitution (pay → read → approve/reject)
app.use('/api/arbiter', arbiterRoutes); // ✅ Arbiter demo endpoints (engagement cycle, held panel, live price)
