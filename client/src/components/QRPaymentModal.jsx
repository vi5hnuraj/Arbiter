...
const QRPaymentModal = ({ isOpen, onClose, qrData, user }) => {
  ...
  const performInternalSend = async (confirm) => {
    ...
    try {
      ...
      toast.loading("Recording transaction log on Arbiter...", { id: toastId });
  ...
  const handleConfirmPayment = async () => {
    ...
    if (walletRail === 'internal') {
      ...
      try {
        const prov = new ethers.providers.JsonRpcProvider(import.meta.env.VITE_RPC_URL || "https://sepolia-rollup.arbitrum.io/rpc");
...
Verified on Blockchain`;
    ...
    link.href = url;
    link.download = `Arbiter_Receipt_${String(successData.txHash).substring(0, 8)}.txt`;
    link.click();
            ...
            <div className="flex flex-col items-center text-center space-y-4 animate-in zoom-in-95 duration-200">
              ...
              <div className="flex flex-col w-full gap-2 pt-2">
                {successData.txHash.startsWith("0x") && (
                  ...
                  >
                    <FiExternalLink size={15} /> View on Arbiscan ↗
                  </a>
                )}
