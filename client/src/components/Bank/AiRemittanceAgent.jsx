...
const AiRemittanceAgent = ({ user: initialUser, refreshData }) => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: 'ai',
      text: "👋 Hi! I'm your Arbiter AI Agent. Powered by Arbiter, I can execute global payments & vault actions for you instantly!\n\n💡 Type `/` or click the command icon below to choose from all tools.",
    }
  ]);
  ...
  const commandTools = [
    ...
    { command: '/find', title: 'Find User', prompt: 'Find user @username', desc: 'Search Arbiter user' },
  ...
  const executeTransactionExternal = async (data, messageId) => {
    ...
    const chainIdHex = '0x' + Number(import.meta.env.VITE_CHAIN_ID || 421614).toString(16);
          ...
          if (e.code === 4902) {
            await mmProvider.request({
              ...
              params: [{
                ...
                chainName: import.meta.env.VITE_CHAIN_NAME || 'Arbitrum Sepolia',
  ...
  const executeAuth = async (data, messageId) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now(),
        sender: 'ai',
        text: `✅ Funds locked in Arbiter Manager! Payment ID: ${contractPaymentId} — ${data.amount} USDC will be released to ${data.recipient} automatically at the scheduled time. No intermediary, no treasury.${feeTxHash ? `\n\n🧾 Transaction fee paid: ${Number(data.feeTotal).toFixed(6)} USDC` : ''}`,
        isSuccess: true
      }]);
