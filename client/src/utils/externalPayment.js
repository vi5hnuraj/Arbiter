...
const ACTIVE_CHAIN_ID = Number(import.meta.env.VITE_CHAIN_ID || 421614);
const ACTIVE_RPC = import.meta.env.VITE_RPC_URL || 'https://sepolia-rollup.arbitrum.io/rpc';
const ACTIVE_EXPLORER = import.meta.env.VITE_EXPLORER_URL || 'https://sepolia.arbiscan.io';
const ACTIVE_CHAIN_NAME = import.meta.env.VITE_CHAIN_NAME || 'Arbitrum Sepolia';
...
// Ensure the connected browser wallet operates on Arbitrum Sepolia (421614). Uses the
// raw EIP-1193 request() API directly (reliable across MetaMask/OKX/WC/Rabby).
// Returns true when the wallet was already on Arbitrum Sepolia, false after a switch.
...
// Rabby / Coinbase / WalletConnect via AppKit). On Arbitrum Sepolia, ETH is gas;
...
export const sendExternalTransfer = async (rawProvider, { to, valueWei, gasLimit }) => {
  if (!rawProvider) {
    throw new Error('External wallet not connected. Connect your MetaMask or Arbitrum-compatible wallet first.');
  }
