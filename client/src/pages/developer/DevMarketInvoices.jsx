...
import {
  ...
  FiCopy, FiChevronDown, FiFilter, FiMoreVertical, FiLock
} from 'react-icons/fi';
...
// ---------------------------------------------------------------------------
// Held escrow purchases (x402 · Held) — escrow buys are purchases too
// ---------------------------------------------------------------------------

const HELD_TONE = {
  RELEASED: { label: 'Paid', cls: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30', dot: 'bg-emerald-400' },
  CANCELLED: { label: 'Refunded', cls: 'bg-sky-500/10 text-sky-300 border-sky-500/30', dot: 'bg-sky-400' },
};

const heldBadge = (e) => {
  if (HELD_TONE[e.status]) return HELD_TONE[e.status];
  return e.delivered
    ? { label: 'In review', cls: 'bg-amber-500/10 text-amber-300 border-amber-500/30', dot: 'bg-amber-400' }
    : { label: 'Awaiting delivery', cls: 'bg-zinc-800/70 text-zinc-400 border-zinc-600/50', dot: 'bg-zinc-500' };
};

const TX_EXPLORER = 'https://sepolia.arbiscan.io/tx/';
const shortAddr = (a) => (a ? `${a.slice(0, 6)}…${a.slice(-4)}` : '—');

const HeldEscrowSection = ({ rows }) => {
  if (!rows.length) return null;
  return (
    <div className="mb-5">
      {/* Section header — same rhythm as the invoice list below */}
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <FiLock size={13} className="text-emerald-400" />
          <p className="text-xs font-semibold text-zinc-300">Held escrow purchases</p>
          <span className="rounded-full border border-zinc-700 bg-zinc-800/60 px-1.5 py-0.5 text-[9px] font-medium text-zinc-400">
            x402 · {rows.length}{rows.length > 8 ? ` · showing 8` : ''}
          </span>
        </div>
        <Link to="/developer/x402" className="text-[11px] font-medium text-zinc-500 transition-colors hover:text-blue-400">
          Open x402 · Held →
        </Link>
      </div>

      {/* Rows — identical card layout to InvoiceRow */}
      <div className="space-y-1">
        {rows.slice(0, 8).map((e) => {
          const meta = heldBadge(e);
          const when = e.createdAt ? new Date(e.createdAt) : null;
          const dateStr = when ? when.toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
          const timeStr = when ? when.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : '';
          const txLink = (hash, label) => (
            <a key={label} href={`${TX_EXPLORER}${hash}`} target="_blank" rel="noreferrer"
              className="text-blue-400/80 transition-colors hover:text-blue-300 hover:underline">
              {label} ↗
            </a>
          );
          return (
            <div key={e.escrowId}
              className="group flex min-h-[72px] items-center gap-3 rounded-xl border border-zinc-800/60 bg-zinc-900/30 px-3 py-2.5 transition-all duration-150 hover:border-zinc-700 hover:bg-zinc-900/60">
              {/* Left: icon + escrow identity + proof links */}
              <div className="flex min-w-0 flex-1 items-center gap-2.5">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-700/80 to-cyan-800/80 text-white">
                  <FiLock size={14} />
                </div>
                <div className="min-w-0">
                  <p className="truncate font-mono text-sm font-semibold text-white" title={e.escrowId}>{e.escrowIdShort}</p>
                  <p className="truncate text-[11px] text-zinc-500">
                    Buyer <span className="text-zinc-300">{shortAddr(e.payer)}</span>
                    {' · '}Provider <span className="text-zinc-300">{shortAddr(e.receiver)}</span>
                  </p>
                  <p className="flex flex-wrap items-center gap-x-2.5 text-[10px] text-zinc-600">
                    {e.settleTx && txLink(e.settleTx, 'settle')}
                    {e.deliveredTx && txLink(e.deliveredTx, 'delivery')}
                    {e.decisionTx && txLink(e.decisionTx, 'decision')}
                    {!e.settleTx && !e.deliveredTx && !e.decisionTx && <span>on-chain escrow</span>}
                  </p>
                </div>
              </div>

              {/* Right: amount · date · delivery · status — same columns as InvoiceRow */}
              <div className="flex shrink-0 items-center gap-4">
                <div className="min-w-[90px] text-right">
                  <p className="font-mono text-sm font-semibold text-white">
                    {e.amount ? fmtBot(e.amount) : '—'} USDC
                  </p>
                  <p className="text-[10px] text-zinc-600">{e.usdCents != null ? `${e.usdCents} ¢ · escrow` : 'escrow'}</p>
                </div>
                ...
                <div className="min-w-[76px] text-right">
                  <p className={`text-[11px] font-medium ${e.delivered ? 'text-emerald-400' : 'text-zinc-500'}`}>
                    {e.delivered ? 'Delivered' : 'Undelivered'}
                  </p>
                  <p className="text-[10px] text-zinc-600">{e.delivered ? 'evidence on-chain' : 'no evidence'}</p>
                </div>
                ...
                <div className="min-w-[92px] text-right">
                  ...
                  {e.decision && <p className="mt-1 truncate text-[9px] text-zinc-600">{e.decision}</p>}
                </div>
              </div>
            </div>
          );
      ...
      {rows.length > 8 && (
        <p className="mt-2 text-[10px] text-zinc-600">
          Showing the 8 most recent of {rows.length} — full history on the{' '}
          <Link className="text-blue-400 hover:underline" to="/developer/x402">x402 · Held</Link> page and on-chain.
        </p>
      )}
    </div>
  );
...
<header><div><h1>Arbiter Invoice</h1><p class="muted">${invoiceNumber(inv.invoiceId)}</p></div><div class="muted">Issued ${fmtWhen(inv.createdAt)}</div></header>
...
<p class="muted" style="margin-top:28px">Generated ${fmtWhen(new Date().toISOString())} · Arbiter Developer Console</p></body></html>`;
...
const DevMarketInvoices = () => {
  ...
  // Escrow buys (x402 · Held) are purchases too — they settle on ArbiterManager
  // instead of through a normal invoice, so they arrive via their own endpoint.
  const heldState = useApi({ fetcher: () => developerApi.arbiterHeldJobs(), deps: [] });

  const invoices = data?.invoices || [];
  const heldEscrows = heldState.data?.escrows || [];
  ...
  return (
    <div>
      ...
      {/* Held escrow purchases (x402 · Held) — shown alongside invoices because
          an escrow buy is still a purchase: same money, different rail. */}
      <HeldEscrowSection rows={heldEscrows} />
