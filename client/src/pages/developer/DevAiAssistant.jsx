  ...
  return (
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      {settlement && <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
        <p className="text-[10px] font-bold uppercase tracking-widest text-cyan-300"><FiDollarSign className="mr-1 inline" size={12} /> Arbiter settlement</p>
...
const ProviderDiscoveryCard = ({ providers, intent }) => {
  ...
  return <div className="mt-4 rounded-xl border border-violet-500/20 bg-violet-500/5 p-4">
    ...
    <p className="mt-3 text-xs leading-5 text-violet-100/70">Trust and earnings are different dimensions. Arbiter ranks providers using settlement reliability and risk, not volume alone.</p>
  </div>;
};
...
export default function DevAiAssistant() {
  ...
  const [messages, setMessages] = useState([{ role: 'assistant', answer: 'I am your Arbiter AI Operator. Give me a goal and I will discover services, evaluate providers with Graph evidence, pay with USDC on Arbitrum, and verify settlement.', steps: [], data: null }]);
  ...
  const resetChat = () => {
    if (busy) return;
    setMessages([{ role: 'assistant', answer: 'I am your Arbiter AI Operator. Give me a goal and I will discover services, evaluate providers with Graph evidence, pay with USDC on Arbitrum, and verify settlement.', steps: [], data: null }]);
    setInput('');
    setError(null);
  };
  ...
  return <div className="mx-auto max-w-7xl space-y-5 pb-6">
    <header>
      <div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-bold text-white">Arbiter AI Operator</h1><span className="inline-flex items-center gap-1.5 rounded-full border border-violet-500/30 bg-violet-500/10 px-2.5 py-1 text-[11px] text-violet-300"><FiZap size={12} /> Graph → Arbitrum → Verify</span></div>
      <p className="mt-1 max-w-3xl text-sm leading-relaxed text-zinc-500">The conversational surface of the same engine: ask in plain language, the operator still settles every payment through Arbiter.</p>
    </header>
