...
import {
  FiArrowLeft, FiRefreshCw, FiSend, FiCpu, FiActivity, FiDownload, FiRefreshCcw, FiExternalLink
} from 'react-icons/fi';
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mt-2">
            <span className="text-[11px] text-zinc-500 flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Arbitrum Sepolia · {agent.chainId || 421614}
            </span>
          ...
          <div className="bg-zinc-900/60 border border-zinc-800/60 rounded-2xl p-4">
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex flex-col gap-1.5 pt-2 border-t border-zinc-800/50">
                {[
                  { label: 'USDC', value: balance.data?.usdc, color: 'text-emerald-400' },
                  { label: 'USDG', value: balance.data?.usdg, color: 'text-cyan-400' },
                  { label: 'ETH', hint: 'gas', value: balance.data?.eth, color: 'text-amber-300' }
                ].map((b) => (
                  <div key={b.label} className="flex items-center justify-between text-xs">
                    <span className="text-zinc-500">
                      {b.label}{b.hint ? <span className="text-zinc-600"> · {b.hint}</span> : null}
                    </span>
                    <span className={`font-mono font-semibold ${b.color}`}>
                      {balance.loading ? '...' : `${Number(b.value ?? 0).toFixed(6)} ${b.label}`}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {history.loading && !history.data ? (
              ...
              <div className="space-y-1.5 max-h-72 overflow-y-auto">
                {txHistory.map((tx) => {
                  ...
                  const signed = /^[-+]/.test(String(tx.amount)) ? tx.amount : `-${tx.amount}`;
                  const isEscrow = tx.kind && tx.kind !== 'wallet send';
                  return (
                  <div key={tx.id} className={`flex items-center justify-between rounded-lg px-3 py-2.5 border ${isEscrow ? 'bg-violet-950/20 border-violet-900/40' : 'bg-zinc-950/40 border-zinc-800/40'}`}>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        {isEscrow && (
                          <span className={`rounded px-1 py-0.5 text-[9px] font-bold uppercase ${isIn ? 'bg-emerald-500/15 text-emerald-300' : 'bg-violet-500/15 text-violet-300'}`}>{isIn ? 'IN' : 'OUT'}</span>
                        )}
                        <code className="font-mono text-[11px] text-zinc-400 truncate">{isEscrow ? `${tx.kind} · ${tx.counterparty || tx.to}` : tx.to}</code>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-zinc-600">{new Date(tx.createdAt).toLocaleString()}</span>
                        {tx.txHash && String(tx.txHash).startsWith('0x') && (
                          <a href={`https://sepolia.arbiscan.io/tx/${tx.txHash}`} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-400 hover:underline inline-flex items-center gap-0.5">
                            Arbiscan <FiExternalLink size={8} />
                          </a>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0 ml-2">
                      <span className={`text-xs font-semibold font-mono ${isIn ? 'text-emerald-400' : 'text-rose-400'}`}>{isIn ? signed.replace('-', '+') : signed}</span>
                      <span className="ml-1 text-[10px] text-zinc-500">{tx.token}</span>
