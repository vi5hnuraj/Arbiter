    ...
    const q = Object.entries(params).length
      ? '?' +
        Object.entries(params)
          ...
          .join('&')
      : '';
    return `curl 'https://api.arbiter.io${ep}${q}' \\
  ...
  ts: (ep, params = {}) => {
    const q = Object.keys(params).length
      ? `?${Object.keys(params)
          ...
          .join('&')}`
      : '';
    return `const res = await fetch(\`https://api.arbiter.io${ep.replace(/:([a-zA-Z_]+)/g, '${$1}')}${q}\`, {
  headers: {
...
export default function DevApi() {
  ...
  return (
    <div className="space-y-5">
        ...
        >
          ...
          {filteredKeys.length ? (
            ...
            <EmptyState
              ...
              description="API keys authorize your agents to call the Arbiter API. Keys can be scoped and restricted by IP."
        ...
        >
          {agents.length ? (
            ...
            <EmptyState
              icon={FiShield}
              title="No agents yet"
              description="Create an AI agent to start transacting with the Arbiter API."
      ...
      <Card
        title="API reference"
        subtitle="Explore every Arbiter endpoint, its required permissions, and live SDK examples"
        actions={
          <Link
            to="/developer/docs"
