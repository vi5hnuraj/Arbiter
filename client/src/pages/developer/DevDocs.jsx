...
const PROD_BASE = import.meta.env.VITE_API_URL?.replace(/\/$/, '') || 'https://api.arbiter.io/api';
...
const SDK_INSTALL = {
  javascript: `npm install arbiter`,
  python: `pip install arbiter`,
  go: `go get github.com/arbiter/arbiter-go`,
  java: `./build.sh   # no Maven required`,
  php: `composer require arbiter/arbiter-php`
};

const SDK_QUICKSTART = {
  javascript: `const { Arbiter } = require('arbiter');

const gp = new Arbiter();
...
const project        = await gp.projects.create({ name: 'My Project', orgId: 'org_xyz' });
  `,
  python: `from arbiter import Arbiter, Agent
...
`,
  go: `import "github.com/arbiter/arbiter-go/arbiter"

client := arbiter.NewClient("") // defaults to production
...
result, _ := agent.Pay(ctx, arbiter.PayParams{To: "0x…", Amount: "0.02"})
history, _ := agent.History(ctx, 50)
stats, _ := agent.Stats(ctx)
rotated, _ := agent.RotateKey(ctx) // returns a new one-time apiKey

// ── V3: Marketplace & Services ──
svc, _ := gp.Services.Publish(ctx, arbiter.PublishParams{
    Name: "Travel AI Booking Service",
    PricePerCall: 0.05,
    SLA: "professional",
})

listings, _ := gp.Marketplace.Browse(ctx, arbiter.BrowseParams{Query: "travel", ServiceType: "orchestrator"})
install, _ := gp.Marketplace.Install(ctx, arbiter.InstallParams{
    ListingID: listings[0].ListingID, ProjectID: "proj_123",
})

// Invoke an installed agent with smart-procurement routing
invocation, _ := agent.Invoke(ctx, arbiter.InvokeParams{
    AgentID: install.AgentID,
    Payload: map[string]interface{}{"action": "book", "destination": "LAX"},
})

// Stateful session
session, _ := agent.Sessions.Create(ctx, arbiter.SessionParams{AgentID: install.AgentID})
session.Invoke(ctx, arbiter.InvokeParams{Payload: map[string]interface{}{"action": "get_status"}})

// ── V3: Commerce ──
usage, _ := agent.ReportUsage(ctx, arbiter.UsageParams{SubscriptionID: "sub_abc", Units: 150})
invoice, _ := agent.GenerateInvoice(ctx, arbiter.InvoiceParams{RecipientID: "org_xyz", Items: []arbiter.LineItem{
...
project, _ := gp.Projects.Create(ctx, arbiter.ProjectParams{Name: "My Project", OrgID: "org_xyz"})
  `,
  java: `Arbiter gp = new Arbiter(); // defaults to production
...
Agent agent = gp.agent(apiKey); // apiKey from the create response
    ...
    `,
  php: `use Arbiter\\Arbiter;

$gp = new Arbiter(); // defaults to production
    ...
    <div className="flex gap-8">
      ...
      <div className="flex-1 min-w-0">
        <header className="mb-6">
          ...
          <p className="text-sm text-zinc-500 mt-1">Build financial rails for autonomous AI agents on Arbitrum Chain.</p>
        ...
        <section id="overview" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Arbiter is <strong className="text-white">financial infrastructure for autonomous AI agents on Arbitrum Chain</strong>.
        ...
        <section id="api-keys" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400">
            Keys are hashed at rest (SHA-256) — Arbiter never stores the raw secret. Two key types: a{' '}
        ...
        <section id="organizations" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Arbiter is a multi-tenant platform. Every API resource is scoped to an {' '}
        ...
        <section id="agents" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            An AI agent is a headless wallet on Arbitrum Chain with its own API key (<code className="text-cyan-400">gpay_sk_…</code>).
        ...
        <section id="payments" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Programmatic payments from agent wallets and developer accounts on Arbitrum Chain.
          </p>
          <div className="mt-4 text-sm text-zinc-400 space-y-1">
        ...
        <section id="wallets" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Every AI agent gets a headless wallet on Arbitrum Chain, provisioned automatically when the agent
        ...
        <section id="sdk" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 mb-4">
            Official client libraries for the Arbiter Agent Payments API — JavaScript, Python, Go, Java and PHP. All zero-dependency.
          </p>
        ...
        <section id="webhooks" className="mb-10 scroll-mt-8">
          ...
          <p className="text-sm text-zinc-400 mb-4">
            ...
            with an HMAC-SHA256 signature in the <code className="text-blue-400">x-arbiter-signature</code> header.
...
// const sig = req.headers['x-arbiter-signature'];
// const ts  = req.headers['x-arbiter-timestamp'];
// if (!verifyWebhookSignature({ secret: process.env.ARBITER_WEBHOOK_SECRET,
