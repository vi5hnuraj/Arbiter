          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            {evidenceLoading ? <div className="mt-6 text-sm text-zinc-500">Loading live Graph evidence…</div> : evidence.error ? <div className="mt-6 text-sm text-amber-400">{evidence.error}</div> : (
              <>
                ...
                <div className="mt-4 flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950/40 px-3 py-2.5 text-xs"><span className="text-zinc-500">Reputation</span><span className="text-zinc-200">{evidence.passport?.onChainRegistered ? 'On-chain record' : 'New provider'} · {evidence.passport?.intelligence?.paymentCount ?? 0} indexed payments</span></div>
                <div className="mt-4 flex items-center justify-between"><span className="text-[11px] text-violet-400">The Graph · Arbitrum Sepolia</span><Link to={`/developer/agent-profile?agentId=${encodeURIComponent(evidence.service.agent?.agentId || evidence.service.provider?.agentId || evidence.service.agentId || '')}&serviceId=${encodeURIComponent(evidence.service.serviceId)}`} onClick={() => setEvidence(null)} className="inline-flex items-center gap-1 text-xs text-violet-400 hover:text-violet-300">View full Passport <FiArrowUpRight size={11} /></Link></div>
              </>
            )}
          </div>
        </div>
    ...
    {s.provider && (
      <div className="mt-3 space-y-2 border-t border-zinc-800/50 pt-3">
        <div className="flex items-center justify-between">
           <Link to={`/developer/marketplace/service/${encodeURIComponent(s.serviceId)}`} className="truncate text-[11px] font-mono text-zinc-600 hover:text-zinc-300" title={s.provider.wallet}>
              {s.providerOrg?.name || s.provider?.name || s.provider?.agentId}
           </Link>
        </div>
        {/* Agent Passport: reputation + settlement in one row */}
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] text-zinc-500">
          ...
          <span className="text-cyan-400">● Arbiter settlement</span>
