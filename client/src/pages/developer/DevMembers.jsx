...
const DevMembers = () => {
  ...
  const invite = async () => {
    ...
    if (!tag) return toast.error('Enter a Arbiter tag (e.g. @username_gl)');
    setBusy(true);
    try {
  ...
  return (
    <div>
      ...
      <Modal open={inviteOpen} onClose={() => { setInviteOpen(false); setTagSearch([]); setInviteTag(''); }} title="Invite member" subtitle="Search by Arbiter tag to send an in-app invitation">
        <label className="block text-xs text-zinc-500 mb-1" htmlFor="invite-tag">Arbiter Tag</label>
