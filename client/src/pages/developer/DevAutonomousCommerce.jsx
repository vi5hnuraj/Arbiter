...
const stages = [
  ...
  { id: 'pay', label: 'Pay on Arbitrum', description: 'Settle within policy', Icon: FiDollarSign },
      ...
      <header className="relative overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 via-zinc-900/70 to-cyan-500/10 p-6 sm:p-8">
        ...
        <div className="relative">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2.5 py-1 text-[11px] text-cyan-300"><FiZap size={12} /> Graph → Decide → Arbiter → Verify</span>
          </div>
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-white sm:text-4xl">Autonomous Commerce</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">Give an agent a goal. It decides who to pay, then <span className="text-cyan-300">Arbiter</span> dry-runs the exact transaction, and only then executes it on Arbitrum Sepolia — nothing is inferred at execution time.</p>
        </div>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)]">
        ...
        <Card title="Policy guardrails" subtitle="Applied to this purchase before any payment decision.">
          <div className="space-y-1">
            ...
            <PolicyRow label="Network" value="Arbitrum Sepolia" />
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card title="Arbiter execution" subtitle="Dry-run simulated first, then executed exactly. Every tx verifiable.">
              ...
              {execution?.approveTxHash && (
                <p className="mt-3 text-xs text-zinc-500">USDC approve: <a className="font-mono text-cyan-400 hover:text-cyan-300" href={`https://sepolia.arbiscan.io/tx/${execution.approveTxHash}`} target="_blank" rel="noreferrer">{execution.approveTxHash.slice(0, 10)}…{execution.approveTxHash.slice(-6)}</a></p>
              )}
              {settlement?.txHash && <a className="mt-3 inline-flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300" href={`https://sepolia.arbiscan.io/tx/${settlement.txHash}`} target="_blank" rel="noreferrer">View transaction on Arbiscan <FiArrowRight size={12} /></a>}
