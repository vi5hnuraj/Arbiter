import {
  ...
  PaymentReleased,
  PaymentCancelled,
} from '../generated/ArbiterPaymentManager/ArbiterPaymentManager';
import { ArbiterPaymentManager } from '../generated/ArbiterPaymentManager/ArbiterPaymentManager';
...
export function handlePaymentCreated(event: PaymentCreated): void {
  ...
  payment.amount = event.params.amount.times(BigInt.fromI32(10).pow(12)); // 6-dec USDC -> 18-dec
  ...
  settlement.amount = event.params.amount.times(BigInt.fromI32(10).pow(12)); // 6-dec USDC -> 18-dec
  settlement.status = HELD;
  settlement.save();
  payment.settlement = id;

  const manager = ArbiterPaymentManager.bind(event.address);
